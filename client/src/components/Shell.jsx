import { Bot, FileText, Gauge, LogOut, Menu, MessageSquarePlus, Settings, Users, X } from "lucide-react";
...
export default function Shell({ children }) {
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${open ? "sidebar-open" : ""}`}>
      ...
      <nav>{links.map(([to, label, Icon]) => <NavLink key={to} to={to} end={to === "/"} onClick={() => setOpen(false)} className={({ isActive }) => `nav-link ${isActive ? "active" : ""}`}>{createElement(Icon, { size: 18 })}<span>{label}</span></NavLink>)}</nav>
